...
export default async function decorate(block) {
  ...
  ${navArray
    .map(
      (nav, index) => `
              <li style="transition-delay: ${index * 0.1}s; height: calc(100vh / ${
        isDesktop ? 1 : navArray.length
      });">
              ${nav.outerHTML}
              </li>
            `
  ...
  // whenever the input from .hamburger is checked add the class "hidden" to main
