function sampleRUM(e, t = {}) {
  ...
  try {
    ...
    let { weight: c, id: n, firstReadTime: d } = window.hlx.rum;
      ...
      if (e.children) {
        ...
        if (a[1]) {
          ...
          if (l.querySelector('a')) {
            ...
            let i = [...l.querySelectorAll('img')];
            r = 1 === i.length ? i[0].src : i.map((e) => e.src);
          } else if (l.querySelector('p')) {
            let c = [...l.querySelectorAll('p')];
            r = 1 === c.length ? c[0].textContent : c.map((e) => e.textContent);
  ...
  return (
    ...
    l.forEach((e, o) => {
      if (o < l.length - 1) {
        ...
        e.media && c.setAttribute('media', e.media),
          c.setAttribute('srcset', `${s}?width=${e.width}&format=${i}&optimize=medium`),
          r.appendChild(c);
      } else {
        ...
        n.setAttribute('loading', a ? 'eager' : 'lazy'),
          ...
          r.appendChild(n),
          n.setAttribute('src', `${s}?width=${e.width}&format=${i}&optimize=medium`),
          n.setAttribute('width', `${e.width}`),
          n.setAttribute('height', `${e.height ?? 'auto'}`);
      }
    }),
    r
  );
